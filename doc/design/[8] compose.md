...
some principles of namespace:
- module developer
  - developer hasn't any feeling/knowing about it
    - during the developing, developer still write the normal naming model
    - using model name by "indexes.ts" file that auto generated by tarat
- module user
  - 如果引入了相同的module，会有明显的感知到区别，但应该只感受一层的差别，不关心内部的深层嵌套的第二层
    - comment: 深层嵌套的第二层（如：module-A）客观已经是第一层（module-B)的部分了，不需要再区分B是由谁组成的，就像是代码复制，而不是模块引用

可能碰到的问题：
- 下层module在编译导出时已经代码写死了indexes的变量名，不灵活
  - 如果model名称会随着使用者而变化，就需要动态机制来注入最新的model name，不能是写死
    - 动态机制：
      - 函数入参
      - 配置文件
      - 环境变量
...
动态model name实现方案思路：
- 不能在代码中写死，也不能影响driver的正常表达，开发者感知到，所以不能是函数入参
  - 因为是前后端一体化的，考虑执行环境，也不能使用环境变量或者文件读写，只能是编程语言的自身feature
    - 方法1：全局变量，通过长的拼接key索引
      - 连续调用key
    - 方法2：局部变量，通过context传递，通过简单key索引
      - scope namespace
      - scope 新增插件注入namespace
