...
`
async function generateReferrenceHooks (c: IConfig, h: IDepencyHook[]) {
  const curHooksDir = path.join(c.cwd, c.hooksDirectory)

  await Promise.all(h.map(obj => new Promise<void>((res, rej) => {
    const code = referrenceHookTemp({ path: obj.modulePath })
    const f = path.join(curHooksDir, `${obj.name}${c.ts ? '.ts' : '.js'}`)
...
export async function composeHook(c: IConfig) {
  const depencyHooks: IDepencyHook[] = []
  
  c.dependencyModules.forEach(moduleName => {
    const dir = path.join(c.cwd, 'node_modules', moduleName)
    const distHooksDir = path.join(dir, c.buildDirectory, c.hooksDirectory)
    ...
    fs.readdirSync(distHooksDir)
      ...
      .forEach(f => {
        ...
        if (depencyHooks.find(v => v.name === hookName)) {
          ...
          if (depencyHooks.find(v => v.name === hookName)) {
            throw new Error('[tarat] can not handle hook name confict betwwen all depency modules')
          }
        }
        depencyHooks.push({
          name: hookName,
          modulePath: `${moduleName}/${c.buildDirectory}/${c.hooksDirectory}/${name}`
        })
      })
  })
  await generateReferrenceHooks(c, depencyHooks)
}
